...
def getdetails(student):
	...
	Examples:
		getdetails(bs4.element.Tag)
		{'Name': 'Sanders, Lia', 'URLs': 'Researchgate: https://www.researchgate.net/profile/Lia_Sanders\n', 'Doctoral Project Title': 'Visual spatio-temporal integration in patients with Schizophrenia', 'Project Description': 'General theories of brain function assert that the brain works like an inference machine, whereby prior expectations are used to derive perceptual experiences from the sensory inputs. Impaired perceptual inference has been suggested as one possible causal factor underlying positive symptoms, and in particular delusions, in schizophrenia. According to this framework, delusional explanations are thought to reflect the attempt to cope with unusual percepts resulting from such impaired perceptual inference. During my PhD, we conducted a series of clinical studies testing this theoretical model. Methods: psychophysics, EEG, fMRI. \r\n', 'Funding': 'Mind and Brain scholarship\nMind and Brain postdoctoral scholarship', 'Supervisors': 'Prof. Dr. Philipp Sterzer\r\nProf. Dr. Andreas Heinz\r\nProf. Dr. Norbert Kathmann', 'M&B topics': 'Topic 1: Perception, attention, consciousness\nTopic 5: Brain disorders and mental dysfunction', 'Degrees obtained': 'MD, Federal University of Ceará – Fortaleza, Brazil', 'Institute': 'Klinik für Psychiatrie, CCM, Charité – Universitätsmedizin Berlin', 'Title': 'Dr.', 'Cohort': '2009', 'Status': 'Alumna', 'Researchgate': 'https://www.researchgate.net/profile/Lia_Sanders'}
    ...
    for tr in project_details.findAll('tr'):
        ...
        elif tr.find('th').text == "Funding":
            towrite = td.text.replace('; ', '\n')
        else:
            towrite = td.text
...
def makedataframe(student_data, columns):
    """
	Creates and poppulates dataframe with scraped data
	...
	Returns:
		pd.Dataframe: poppulated dataframe 
